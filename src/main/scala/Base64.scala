...
object Base64 {
  ...
  class B64Scheme(val encodeTable: IndexedSeq[Char], padding: Boolean = true) {
  ...
  implicit class Encoder(b: Array[Byte]) {
    private[this] val basic = Array(127, 127, 127).map(_.toByte)
    ...
    def toBase64(implicit scheme: B64Scheme = base64): String = {
      ...
      val pad = (scheme.pad - b.length % scheme.pad) % scheme.pad
  ...
  implicit class Decoder(s: String) {
    ...
    def toByteArray(implicit scheme: B64Scheme = base64): Array[Byte] = {
      if (scheme.pad == 3) {
        ...
        if (pad > 0) throw new java.lang.IllegalArgumentException("Invalid Base64 String:" + s)
      }
